...
void ConnectionSocket::openConnection(std::string address, uint16_t port, std::string secret, bool ipv6, int32_t networkType)
{
    ...
    {
        if (LOGS_ENABLED)
            DEBUG_D("connection(%p) connecting via proxy %s:%d secret[%d]", this, proxyAddress->c_str(), proxyPort, (int)proxySecret->size());
...
void ConnectionSocket::openConnectionInternal(bool ipv6)
{
    ...
    {
        if (LOGS_ENABLED)
            DEBUG_E("connection(%p) set SO_SNDBUF failed", this);
    }
    if (setsockopt(socketFd, SOL_SOCKET, SO_RCVBUF, &size, sizeof(int)))
    {
        if (LOGS_ENABLED)
            DEBUG_E("connection(%p) set SO_RCVBUF failed", this);
    }
#endif
        ...
        else
        {
            ...
            while (true)
            {
                ...
                if (readCount > 0)
                {
                    ...
                    {
                        if (LOGS_ENABLED)
                            DEBUG_D("connection(%p) TLS received %d", this, (int)readCount);
                        size_t newBytesRead = bytesRead + readCount;
                        if (newBytesRead > 64 * 1024)
                        {
                            closeSocket(1, -1);
                            if (LOGS_ENABLED)
                                DEBUG_E("connection(%p) TLS client hello too much data", this);
                            return;
                        }
                        if (newBytesRead >= 16)
                        {
                            ...
                            if (std::memcmp(hello1.data(), tempBuffer->bytes, hello1.size()) != 0)
                            {
                                closeSocket(1, -1);
                                if (LOGS_ENABLED)
                                    DEBUG_E("connection(%p) TLS hello1 mismatch", this);
                                return;
                            }
                            ...
                            if (len1 > 64 * 1024 - 5)
                            {
                                closeSocket(1, -1);
                                if (LOGS_ENABLED)
                                    DEBUG_E("connection(%p) TLS len1 invalid", this);
                                return;
                            }
                            else if (newBytesRead < len1 + 5)
                            {
                                if (LOGS_ENABLED)
                                    DEBUG_D("connection(%p) TLS client hello wait for more data", this);
                            ...
                            if (std::memcmp(hello2.data(), tempBuffer->bytes + 5 + len1, hello2.size()) != 0)
                            {
                                closeSocket(1, -1);
                                if (LOGS_ENABLED)
                                    DEBUG_E("connection(%p) TLS hello2 mismatch", this);
                                return;
                            }
                            ...
                            if (len2 > 64 * 1024 - len1 - 5 - 11)
                            {
                                closeSocket(1, -1);
                                if (LOGS_ENABLED)
                                    DEBUG_E("connection(%p) TLS len2 invalid", this);
                                return;
                            }
                            ...
                            if (std::memcmp(tempBuffer->bytes + 64 * 1024, tempBuffer->bytes + 64 * 1024 + 32, 32) != 0)
                            {
                                ...
                                closeSocket(1, -1);
                                if (LOGS_ENABLED)
                                    DEBUG_E("connection(%p) TLS hash mismatch", this);
                                return;
                            }
                            if (LOGS_ENABLED)
                                DEBUG_D("connection(%p) TLS hello complete", this);
                            tlsState = 1;
                            proxyAuthState = 0;
                            bytesRead = 0;
                            adjustWriteOp();
                                ...
                                if (newBytesRead >= 5)
                                {
                                    if (tlsBuffer == nullptr || !tlsBufferSized)
                                    {
                                        ...
                                        if (std::memcmp(header1.data(), header, header1.size()) != 0)
                                        {
                                            closeSocket(1, -1);
                                            if (LOGS_ENABLED)
                                                DEBUG_E("connection(%p) TLS response header1 mismatch", this);
                                            return;
                                        }
                                        ...
                                        if (len1 > 64 * 1024)
                                        {
                                            closeSocket(1, -1);
                                            if (LOGS_ENABLED)
                                                DEBUG_E("connection(%p) TLS response len1 invalid", this);
                                            return;
                                        }
                                        else
                                        {
                                    ...
                                    {
                                        if (LOGS_ENABLED)
                                            DEBUG_D("connection(%p) TLS response new data %d", this, buffer->remaining());
                                    }
                                    ...
                                    {
                                        if (LOGS_ENABLED)
                                            DEBUG_D("connection(%p) TLS response wait for more data, total size %d, left %d", this, tlsBuffer->limit(), tlsBuffer->remaining());
                                    }
                                }
                                else
                                {
                                    ...
                                    tlsBuffer->writeBytes(buffer);
                                    if (LOGS_ENABLED)
                                        DEBUG_D("connection(%p) TLS response wait for more data, not enough bytes for header, total = %d", this, (int)tlsBuffer->position());
                                }
                            }
                        }
                        else
        ...
        else
        {
            if (proxyAuthState != 0)
            {
                if (proxyAuthState >= 10)
                {
                    if (proxyAuthState == 10)
                    {
                        ...
                        {
                            if (LOGS_ENABLED)
                                DEBUG_E("connection(%p) too much data for padding", this);
                            closeSocket(1, -1);
                            return;
                        }
                        uint32_t outLength;
            ...
            else
            {
                if (!onConnectedSent)
                {
                    lastEventTime = ConnectionsManager::getInstance(instanceNum).getCurrentTimeMonotonicMillis();
                    if (LOGS_ENABLED)
                        DEBUG_D("connection(%p) reset last event time, on connect", this);
                    onConnected();
...
void ConnectionSocket::setTimeout(time_t time)
{
    ...
    lastEventTime = ConnectionsManager::getInstance(instanceNum).getCurrentTimeMonotonicMillis();
    if (LOGS_ENABLED)
        DEBUG_D("connection(%p) set current timeout = %lld", this, (long long)timeout);
}
...
bool ConnectionSocket::checkTimeout(int64_t now)
{
    if (timeout != 0 && (now - lastEventTime) > (int64_t)timeout * 1000)
    {
        ...
        else
        {
            lastEventTime = ConnectionsManager::getInstance(instanceNum).getCurrentTimeMonotonicMillis();
            if (LOGS_ENABLED)
                DEBUG_D("connection(%p) reset last event time, no requests", this);
        }
    }
    return false;
}
